...
inline
const op_data<T1, op_median>
median(const arma_base<typename T1::elem_type,T1>& X, const u32 dim = 0)
  {
  arma_extra_debug_sigprint();
  
  return op_data<T1, op_median>(X.get_ref(), dim, 0);
  }
...
inline
eT
median(const basic_rowvec<eT>& A)
  {
  arma_extra_debug_sigprint();
...
inline
eT
median(const basic_colvec<eT>& A)
  {
  arma_extra_debug_sigprint();
...
inline
std::complex<T>
median(const basic_rowvec< std::complex<T> >& A)
  {
  arma_extra_debug_sigprint();
...
inline
std::complex<T>
median(const basic_colvec< std::complex<T> >& A)
  {
  arma_extra_debug_sigprint();
