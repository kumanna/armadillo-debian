...
struct sort_index_result_type_deducer< basic_colvec<eT> >
  {
...
template<>
template<typename eT>
struct sort_index_result_type_deducer< basic_rowvec<eT> >
  {
...
sort_index(const arma_base_vec<typename T1::elem_type,T1>& X, const u32 sort_type = 0)
  {
  arma_extra_debug_sigprint();
