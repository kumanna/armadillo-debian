...
inline
const op_data<T1, op_min>
min(const arma_base<typename T1::elem_type,T1>& X, const u32 dim = 0)
  {
  arma_extra_debug_sigprint();
  
  return op_data<T1, op_min>(X.get_ref(), dim, 0);
  }
...
min(const op_data<T1, op_min>& in)
  {
  arma_extra_debug_sigprint();
...
template<typename T1>
inline
const op_data< op_data<T1, op_min>, op_min>
min(const op_data<T1, op_min>& in, const u32 dim)
  {
  arma_extra_debug_sigprint();
  
  return op_data< op_data<T1, op_min>, op_min>(in, dim, 0);
  }
...
inline
eT
min(const op_data<subview<eT>, op_min>& in)
  {
  arma_extra_debug_sigprint();
