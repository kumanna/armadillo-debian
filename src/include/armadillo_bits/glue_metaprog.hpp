...
//! calculates the number of glue_data<Tx,Ty, glue_type> instances on the left hand side argument of glue_data<Tx,Ty, glue_type>
...
struct depth_lhs< glue_type, glue_data<T1,T2,glue_type> >
  {
...
//! fills a given array with addresses of matrices from a recursive instance of glue_data<Tx,Ty, glue_type>.
//! While parsing the recursive instance, if encountered objects are of type op_data<..>,
...
struct mat_ptrs
  ...
  get_ptrs
    (
    const basic_mat<elem_type>** ptrs,
    bool* del,
    const T1& X
    )
    {
    ...
    ptrs[0] = 
      ...
      is_basic_mat<T1>::value ?
        reinterpret_cast<const basic_mat<elem_type>*>(&X)
      :
        new basic_mat<elem_type>(X)
      );
...
template<typename glue_type, typename T1, typename T2>
struct mat_ptrs<glue_type, glue_data<T1,T2,glue_type> >
  {
  typedef typename T1::elem_type elem_type;
  ...
  get_ptrs
    (
    const basic_mat<elem_type>** in_ptrs,
    bool* del,
    ...
    in_ptrs[num]  = 
      (
      is_basic_mat<T2>::value ?
        reinterpret_cast<const basic_mat<elem_type>*>(&X.B)
      :
        new basic_mat<elem_type>(X.B)
      );
...
struct mat_ptrs_outcheck
  ...
  get_ptrs
    ...
    bool* del,
    const T1& X,
    const basic_mat<elem_type>* out_ptr
    )
    {
    ...
    const bool same_ptr = 
      ...
      is_basic_mat<T1>::value ?
        (
        (out_ptr == reinterpret_cast<const basic_mat<elem_type>*>(&X)) ?
          true
        :
          false
        )
...
template<typename glue_type, typename T1, typename T2>
struct mat_ptrs_outcheck<glue_type, glue_data<T1,T2,glue_type> >
  {
  typedef typename T1::elem_type elem_type;
  ...
  get_ptrs
    ...
    const bool same_ptr =
      ...
      is_basic_mat<T2>::value ?
        (
        (out_ptr == reinterpret_cast<const basic_mat<elem_type>*>(&X.B)) ?
          true
        :
          false
        )
