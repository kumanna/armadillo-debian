...
norm(const arma_base<typename T1::elem_type,T1>& X, const u32 k)
  {
...
inline
typename T1::elem_type
norm(const glue_data<T1,T2,glue_minus>& X, const u32 k)
  {
  arma_extra_debug_sigprint();
...
inline
typename T1::elem_type
norm(const glue_data<T1,T2,glue_plus>& X, const u32 k)
  {
  arma_extra_debug_sigprint();
