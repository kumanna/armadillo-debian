...
field<oT>::~field()
  {
  arma_extra_debug_sigprint(arma_boost::format("this = %x") % this);
...
//! construct the field to have a user specified number of elements, assuming a column-major layout
template<typename oT>
inline
...
//! construct the field to have user specified dimensions
template<typename oT>
inline
...
//! change the field to have a user specified number of elements, assuming a column-major layout (data is not preserved)
template<typename oT>
inline
void
...
//! change the field to have user specified dimensions (data is not preserved)
template<typename oT>
inline
void
...
field<oT>::rows(const u32 in_row1, const u32 in_row2)
  ...
  arma_debug_check( ((in_row1 >= n_rows) || (in_row2 >= n_rows) ), "field::rows(): out of bounds" );
  arma_debug_check( (in_row1 >  in_row2), "field::rows(): incorrect usage" );
...
field<oT>::cols(const u32 in_col1, const u32 in_col2)
  ...
  arma_debug_check( ((in_col1 >= n_cols) || (in_col2 >= n_cols) ), "field::cols(): out of bounds" );
  arma_debug_check( (in_col1 >  in_col2), "field::cols(): incorrect usage" );
...
//! creation of subview_field (subfield)
template<typename oT>
inline
subview_field<oT>
field<oT>::subfield(const u32 in_row1, const u32 in_col1, const u32 in_row2, const u32 in_col2)
  ...
  arma_debug_check
    ...
    (in_col1 >  in_col2)
    ,
    "field::subfield(): incorrect usage"
    );
...
inline
void
field<oT>::save(const std::string name) const
  {
  arma_extra_debug_sigprint();
  
  diskio::save_field(*this, name);
  }
...
inline
void
field<oT>::load(const std::string name)
  {
  arma_extra_debug_sigprint();
  
  diskio::load_field(*this, name);
  }
...
inline
void
field_aux::reset_objects(field< basic_mat<eT> >& x)
  {
  arma_extra_debug_sigprint();
...
inline
void
field_aux::reset_objects(field< basic_colvec<eT> >& x)
  {
  arma_extra_debug_sigprint();
...
inline
void
field_aux::reset_objects(field< basic_rowvec<eT> >& x)
  {
  arma_extra_debug_sigprint();
