...
//! Note that as 'glue_data' can be one of the operands, more than one matrix can be stored.
//!
//! For example, we could have:
//! op_data< glue_data< mat, mat, glue_times >, op_trans >

template<typename T1, typename op_type>
class op_data : public arma_base<typename T1::elem_type, op_data<T1, op_type> >
  {
  public:
  ...
  inline explicit op_data(const T1& in_m);
  inline          op_data(const T1& in_m, const elem_type in_aux);
  inline          op_data(const T1& in_m, const u32 in_aux_u32_a, const u32 in_aux_u32_b);
  inline          op_data(const u32 in_aux_u32_a, const u32 in_aux_u32_b, const char junk);
  inline         ~op_data();
